...
var clientSecret = "1484b22fc89acee73ce2f134f00335d567fead80";
    ...
    if ($("#index_hot_div").length > 0) {
        ...
        ajaxReqForGitHub(repoIssuesUrl + "?per_page=10&sort=comments",function (result) {
            ...
            $.each(result, function (i, item) {
                ...
                hotContent += "<a href =\"" + item.body.substr(0, item.body.indexOf("\n") - 1) + "\"target=\"_blank\"" + classDiv + ">" + item.title.substr(0, item.title.indexOf("-") - 1) + "&nbsp;🔥" + (item.comments) + "</a>&nbsp;&nbsp;"
            })
            hotDiv.html("");
            if (hotContent == "") {
                hotDiv.append("无数据记录！");
